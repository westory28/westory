...
export const db={};
export const doc=()=>({});
export const getDocFromServer=async()=>{throw Error("Unexpected maintenance read");};
...
try {
  ...
  assert.equal(await page.evaluate(() => window.fixtureCommands.length), 0);
  await activate.click();
  ...
  await scoped("2027", "1");
  results.checks.push(
    "Removed menu save stays absent; view/target selection and cancel cause no writes; explicit confirmed activation changes student semester",
  );
