...
const entry=`import React from 'react';import {createRoot} from 'react-dom/client';import {HashRouter} from 'react-router-dom';import MainLayout from './src/components/layout/MainLayout';import {LessonTreePanel} from './src/pages/teacher/components/LessonEditorPanels';
createRoot(document.getElementById('root')).render(<HashRouter><MainLayout><h1>수업 자료 확인</h1><button>본문 동작</button><LessonTreePanel treeData={[{id:"qa",title:"검증용 자료",children:[]}]} sidebarOpen={false} onCloseSidebar={()=>{}} onOpenRootModal={()=>{}} onSaveTree={()=>{}} renderTreeNode={node=><button>{node.title}</button>}/></MainLayout></HashRouter>);`;
...
try{
 ...
 for(const width of [390,768,1024,1280,1440]){
  ...
  const menu=top.getByRole('button',{name:'학습 자료 관리 하위 메뉴',exact:true});
 ...
 const toggle=touchPage.getByRole('navigation',{name:'교사 메인 메뉴',exact:true}).getByRole('button',{name:'학습 자료 관리 하위 메뉴',exact:true});
