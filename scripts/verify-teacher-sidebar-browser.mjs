...
try{
 ...
 for(const width of [390,768,1440]){await page.setViewportSize({width,height:900});await page.goto(`http://127.0.0.1:${server.address().port}/#/teacher/loading`);await page.reload();const card=page.getByRole('status');await card.waitFor();const b=await card.boundingBox();assert.ok(Math.abs(b.x+b.width/2-width/2)<2);assert.ok(Math.abs(b.y+b.height/2-450)<2);assert.equal(await card.locator('.animate-spin').evaluate(e=>getComputedStyle(e).animationName),'spin');await page.screenshot({path:join(output,`loading-${width}.png`)});}
