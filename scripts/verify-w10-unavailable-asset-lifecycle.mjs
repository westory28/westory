...
const projectId = "demo-westory-session-w10-assets";
...
const exactFirestoreProcesses = () => {
  if (process.platform === "win32") {
    ...
    assert.equal(query.status, 0, "Could not inspect W10 asset emulator processes.");
...
assert.deepEqual(
  ...
  [],
  "An exact W10 asset emulator process already exists before the suite.",
);
...
assert.deepEqual(
  occupiedBefore,
  [],
  "Emulator ports are occupied before W10 asset rules.",
);
  ...
  process.platform === "win32"
    ? spawnSync(
        ...
        [
          ...
          "/s",
          "/c",
          "npm run verify:w10-unavailable-asset-rules:raw",
        ],
        { cwd: process.cwd(), env: process.env, stdio: "inherit" },
      )
    : spawnSync("npm", ["run", "verify:w10-unavailable-asset-rules:raw"], {
        cwd: process.cwd(),
        env: process.env,
        stdio: "inherit",
      });
...
assert.deepEqual(
  exactFirestoreProcesses(),
  [],
  "W10 asset emulator orphan remains.",
);
assert.deepEqual(
  occupiedAfter,
  [],
  "W10 asset emulator ports remain occupied.",
);
assert.equal(result.error, undefined, "Firebase emulator process could not start.");
assert.equal(
  result.status,
  0,
  `W10 unavailable asset rules suite failed with ${result.status}.`,
);

console.log(
  JSON.stringify({
    suite: "w10-unavailable-asset-lifecycle",
    passed: true,
