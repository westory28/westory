...
const projectId = "demo-westory-session-w9";
...
const result =
  process.platform === "win32"
    ? spawnSync(
        ...
        ["/d", "/s", "/c", "npm run verify:w9-emulator-suite:raw"],
        { cwd: process.cwd(), env: process.env, stdio: "inherit" },
      )
    : spawnSync("npm", ["run", "verify:w9-emulator-suite:raw"], {
        cwd: process.cwd(),
        env: process.env,
        stdio: "inherit",
      });
...
const exactFirestoreProcesses = () => {
  if (process.platform === "win32") {
    ...
    assert.equal(query.status, 0, "Could not inspect W9 emulator processes.");
...
assert.deepEqual(
  ...
  [],
  "Exact W9 Firestore emulator orphan remains.",
);
assert.deepEqual(occupiedPorts, [], "W9 emulator ports remain occupied.");
assert.equal(result.error, undefined, "Firebase emulator process could not start.");
assert.equal(result.status, 0, `W9 emulator suite failed with ${result.status}.`);

console.log(
  JSON.stringify({
    suite: "w9-emulator-lifecycle",
    passed: true,
