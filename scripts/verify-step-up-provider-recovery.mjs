...
export const beginApplicationSessionReauthentication=async()=>{window.trace.push('begin');};
...
export const getIdToken=async()=>{window.trace.push('token');if(window.mode==='hold-ready'){window.releaseReady=ready;}else ready();return 'synthetic-token';};
...
try {
  ...
  for (const viewport of [{width:390,height:844},{width:768,height:1024},{width:1024,height:768},{width:1280,height:800},{width:1600,height:900}]) {
    ...
    for (const method of ["password", "google"]) {
      for (const mode of ["success", "switch-reauth", "switch-session", "hold-ready", method === "password" ? "wrong-password" : "popup-cancel", "session-error"]) {
        ...
        if (["success", "hold-ready"].includes(mode)) {
          ...
          assert.equal((await page.evaluate(() => window.trace)).includes("token"), false, "do not refresh the old user's token");
        } else {
