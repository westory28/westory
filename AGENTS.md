...
## 수정 원칙
- UI·UX의 최우선 기준은 사용자가 정한 Simple & Intuitive Design이다. 사용자의 구체적인 지시와 참고 화면, 기존 Westory 운영 화면, 참고 스킬 순으로 적용한다.
- 메뉴·섹션 아래에 사용자가 요청하지 않은 설명 문단을 추가하지 않는다. 짧은 제목·라벨·상태·행동으로 이해되게 만들고, 설명은 사용자가 요청하거나 직접 넣을 수 있는 기능을 요청했을 때만 추가한다. 오류와 실행 확인은 필요한 사실만 간결하게 표시한다.
- `education-web-design-system`은 접근성·반응형·일관성 점검을 위한 참고 자료다. 새 배치·토큰·디자인을 도입하는 근거로 사용하지 않는다.
- 현재 교사 대시보드의 넉넉한 달력 너비를 유지한다. 공통 화면 폭 제한이나 후속 UI 패치로 축소하지 않고 `UI_RULES.md`의 너비 회귀 기준을 검증한다.
- Superloopy는 현재 요청에서 사용자가 명시적으로 요청한 경우에만 사용한다. 일반 UI 작업에서는 전용 상태·evidence·review 절차를 만들지 않는다.
- 반드시 최소 범위만 수정한다. 관련 없는 정리와 리네임을 끼워 넣지 않는다.
- 기능 로직 변경과 UI 개편은 기본적으로 분리한다.
- 복잡한 작업, 넓은 범위의 조사, 리팩터링, 디버깅, 검증이 필요한 작업에서는 가능한 경우 서브 에이전트나 병렬 조사 도구를 적극 활용한다.
...
- UI·UX는 반드시 기존 Westory 운영 화면과 사용자가 익숙한 조작 방식을 기준으로 한다. 메뉴 구조, 화면 배치, 문구, 버튼, 폼, 표, 색상과 간격을 재사용하고, 기능 추가를 이유로 새로운 디자인이나 사용 흐름을 도입하지 않는다.
- `education-web-design-system`은 기존 Westory 스타일을 유지하기 위한 점검 기준으로 적용한다. 스킬의 일반 권장사항이나 새 토큰보다 기존 운영 화면과 사용자의 명시적 지시가 우선한다.
...
- 검증·권한·충돌 문제가 없으면 커밋·푸시에서 멈추지 않고 운영 배포와 실제 반영 확인까지 한 번에 완료한다. 운영 소스와 도메인 연결을 확인하고 `docs/runbooks/production-release.md`가 있으면 해당 절차를 따른다.
