...
- 학생 `학습` 끝: 학습 현황, 일정, 학사 일정, 출석, 공지·소통, 지난 학기
- 교사 `학습 자료 관리` 끝: 학습 운영, 일정, 공휴일 동기화(관리자 전용), 출석, 공지·소통
- 관리자 학기 전환은 관리자 설정 안의 기존 동선에서 연결한다.
- quiz/history-classroom 실행 route는 부모 화면의 context action으로 접근한다.
- `/student/calendar`처럼 router에는 있으나 기존 메뉴에 없던 호환 화면도 기존 dropdown 끝에서 정상 접근할 수 있게 하며, 직접 URL만 남기지 않는다.
