...
- 실제 운영 조회에서 정기시험 전체 통계가 서답형 80점과 논술형 20점을 합산하면서 만점은 80점으로 표시하는 결함도 확인했다. 전체 통계의 만점을 선택한 두 점수표의 합으로 맞춰 평균 비율·만점 라벨·분포곡선이 동일한 기준을 사용하도록 수정하고, 개별 영역 및 수행평가 통계 회귀를 검증했다.
