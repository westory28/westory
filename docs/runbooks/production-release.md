...
5. 후보 파일로 아래 영수증을 만든다. 실제 변경 경로의 브라우저·회귀 검증을 확인한 뒤 해당 배포 ID를 승격한다.
...
문제 발생 시 직전 검증 배포 ID를 확인해 Vercel에서 승격하고 동일한 파일 대조를 수행한다. 프런트 배포를 되돌려도 Firestore 데이터·Rules·Functions는 자동으로 되돌아가지 않는다. 해당 변경이 포함된 배포에서는 서버와 데이터 호환성부터 확인한다. 운영 학생 접근 개방은 별도 운영 결정으로 다룬다.
