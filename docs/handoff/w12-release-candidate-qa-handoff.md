...
## 2026-09-10 현재 QA 기준 추가

아래의 2026-08-12 내용은 당시 W11 인수인계 기록입니다. 현재 QA에는 이 절의 기준을 우선 적용합니다. 과거 측정 결과를 소급해서 변경하지 않습니다.

- 현재 라우트: raw 48, alias 4, canonical 44(학생 22, 교사 15, 관리자 2, 공개·지원 5).
- 현재 viewport: 390×844, 768×1024, 1024×768, 1440×900, 1600×900. 전체 화면 검증은 아직 남아 있습니다.
- 학기 준비도 구현은 `src/lib/semesterCore.ts` 및 `functions/semesterCore.js` 기준입니다. 삭제된 semesterReadiness 파일을 참조하지 않습니다.
- 수업자료의 판서 도구·저장·복원은 제거합니다. 기존 판서 데이터는 보존합니다. 빈칸 저장과 정답·오답 효과, 핵심포인트 찾기·보상, 이미지·글 각주와 링크를 유지합니다.
- 수업자료 전용 일회성 업로드 ticket 및 저장 Gateway의 로컬 구현·검증은 `docs/w10p-patch-local-handoff.md`를 참고합니다. 지도·사료 도메인의 업로드 제한을 해제한 것은 아닙니다. 실제 Staging 업로드는 별도 검증해야 합니다.
- 새 로컬 타입 검증: 기존 오류 44개 / 12파일, 신규 0. 이는 TypeScript 전체 오류가 0이라는 의미가 아닙니다.
- 이 추가 기록으로 W12 release gate나 운영 배포가 승인되지는 않습니다.
