...
기존 Westory 운영 화면의 UI·UX를 반드시 우선한다. 사용자가 명시적으로 개편을 요청하지 않으면 메뉴 구조·화면 배치·용어·버튼·폼·표·상호작용을 재설계하지 않는다. 아래 일반 토큰과 권장 패턴은 기존 스타일을 유지하기 위한 점검 기준이며, 새 기능은 기존 컴포넌트와 스타일로 필요한 부분만 확장한다.


학생 명단의 학년·반·번호 열은 각각 표 너비의 14%를 사용하고 이름은 왼쪽 정렬한다. 관리 열은 모바일 7rem, 768px 이상 11rem이며 편집·삭제 버튼은 최소 44px 터치 영역을 확보한다. 768px 이상에서는 아이콘과 명칭을 함께 표시한다.
