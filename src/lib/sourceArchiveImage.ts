...
type PreparedSourceArchiveImageVariant = Omit<
...
const renderVariant = async (
  ...
  const scale = Math.min(
    1,
    variant.maxSide / Math.max(image.width, image.height),
  );
  ...
  } satisfies PreparedSourceArchiveImageVariant;
};

export const buildSourceArchiveUpload = async (
  file: File,
): Promise<PreparedSourceArchiveUpload> => {
  ...
  let lastResult: PreparedSourceArchiveImageVariant | null = null;
