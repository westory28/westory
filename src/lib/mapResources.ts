...
export const DEFAULT_GOOGLE_MAP_RESOURCE: MapResource = {
  ...
  tabGroup: "구글 지도",
  description:
    "Google Maps Embed API를 연결하면 원하는 지명을 바로 검색할 수 있습니다.",
  type: "google",
...
export const getPdfSectionTagOptions = (sections: PdfTagSection[]) =>
  normalizeRegionTags(sections.flatMap((section) => section.tags || []));

const normalizePdfTagSections = (sections: unknown): PdfTagSection[] => {
  const defaults = DEFAULT_PDF_TAG_SECTIONS.map((section) => ({
    ...section,
    tags: [...section.tags],
  }));
    ...
    .map((section, index) => ({
      ...
      label: String(
        section && typeof section === "object" && "label" in section
          ? (section as { label?: string }).label
          : "",
      ).trim(),
      tags: normalizeRegionTags(
        section && typeof section === "object" && "tags" in section
          ? (section as { tags?: unknown }).tags
          : [],
      ),
    }))
...
const normalizePdfBlanks = (blanks: unknown): PdfMapBlank[] => {
  ...
  return blanks
    .map((blank, index) => ({
      id:
        String(
          blank && typeof blank === "object" && "id" in blank
            ? (blank as { id?: string }).id
            : "",
        ).trim() || `blank-${index + 1}`,
      page: Math.max(
        1,
        Number(
          blank && typeof blank === "object" && "page" in blank
            ? (blank as { page?: unknown }).page
            : 1,
        ) || 1,
      ),
      left:
        Number(
          blank && typeof blank === "object" && "left" in blank
            ? (blank as { left?: unknown }).left
            : 0,
        ) || 0,
      top:
        Number(
          blank && typeof blank === "object" && "top" in blank
            ? (blank as { top?: unknown }).top
            : 0,
        ) || 0,
      width: Math.max(
        1,
        Number(
          blank && typeof blank === "object" && "width" in blank
            ? (blank as { width?: unknown }).width
            : 1,
        ) || 1,
      ),
      height: Math.max(
        1,
        Number(
          blank && typeof blank === "object" && "height" in blank
            ? (blank as { height?: unknown }).height
            : 1,
        ) || 1,
      ),
      answer: String(
        blank && typeof blank === "object" && "answer" in blank
          ? (blank as { answer?: string }).answer
          : "",
      ).trim(),
      prompt: String(
        blank && typeof blank === "object" && "prompt" in blank
          ? (blank as { prompt?: string }).prompt
          : "",
      ).trim(),
      source:
        blank &&
        typeof blank === "object" &&
        "source" in blank &&
        (blank as { source?: string }).source === "ocr"
          ? ("ocr" as const)
          : ("manual" as const),
    }))
...
export const normalizeMapResource = (
  id: string,
  raw: Partial<MapResource>,
): MapResource => {
  ...
  return {
    ...
    storagePath:
      ...
      deriveStoragePathFromUrl(raw.fileUrl),
    ...
    pdfRegions: Array.isArray(raw.pdfRegions)
      ? raw.pdfRegions
          .map((region) => ({
            ...
            tags: normalizeRegionTags(region?.tags).filter((tag) =>
...
export const groupMapResourcesForDisplay = <T extends MapResource>(
  resources: T[],
): MapResourceDisplayGroup<T>[] => {
