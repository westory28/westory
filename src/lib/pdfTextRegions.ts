...
const resolveLabelBounds = (
  ...
  const width = Math.max(
    averageCharWidth * Math.max(label.length, 1),
    segment.height * 1.15,
  );
...
export const extractPdfTextRegions = (
  ...
  const flush = () => {
    ...
    if (isLikelyRegionLabel(label)) {
      ...
      regions.push({
        ...
        top:
          Math.max(0, pageHeight - current.baselineY - current.height) * scale,
  ...
  items.forEach((item) => {
    ...
    const width = Math.max(
      Number(item.width) || 0,
      Math.abs(scaleX) * Math.max(text.trim().length, 1),
    );
    ...
    const sameLine =
      Math.abs(y - current.baselineY) <=
      Math.max(current.height, height) * 0.45;
    const gap = left - current.right;
    const isJoinable =
      sameLine && gap <= Math.max(8, Math.max(current.height, height) * 0.55);
