...
export interface StudentRankPromotionPreview {
  ...
  previewEmojiEntries: ReturnType<
    typeof getStudentRankPromotionPreviewEmojiEntries
  >;
...
): PointRankTierCode | null => {
  const raw = String(value || "").trim();
  return /^tier_\d+$/.test(raw) ? (raw as PointRankTierCode) : null;
};

export const getStudentRankPromotionStorageKey = (
  config: ConfigLike,
  uid: string,
) => {
...
export const readStudentRankPromotionTierCode = (
  config: ConfigLike,
  uid: string,
) =>
  normalizeTierCode(
    readLocalOnly(getStudentRankPromotionStorageKey(config, uid)),
  );

export const writeStudentRankPromotionTierCode = (
  config: ConfigLike,
  uid: string,
...
export const isStudentRankPromotionEligible = (
  ...
  const previousIndex = previousTierCode
    ? getPointRankTierPosition(rankPolicy, previousTierCode)
    : -1;
  const currentIndex = currentTierCode
    ? getPointRankTierPosition(rankPolicy, currentTierCode)
    : -1;
...
export const buildStudentRankPromotionPreview = (
  ...
  const targetTier =
    resolvedPolicy.tiers.find((tier) => tier.code === requestedTierCode) ||
    resolvedPolicy.tiers[1] ||
    resolvedPolicy.tiers[0] ||
    null;
  const effectLevel =
    resolvedPolicy.celebrationPolicy?.effectLevel === "subtle"
      ? "subtle"
      : "standard";
  const celebrationEnabled =
    resolvedPolicy.celebrationPolicy?.enabled !== false;
  ...
  const targetIndex = resolvedPolicy.tiers.findIndex(
    (tier) => tier.code === targetTier.code,
  );
  const previousTier =
    resolvedPolicy.tiers.find(
      (tier) => tier.code === requestedPreviousTierCode,
    ) ||
