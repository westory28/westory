...
const buildEmojiValueAlias = (
  entry: Omit<PointRankEmojiRegistryEntry, "value">,
): PointRankEmojiRegistryEntry => ({
  ...entry,
  value: entry.emoji,
});

const DEFAULT_PROFILE_EMOJI_REGISTRY_BASE: Array<
  Omit<PointRankEmojiRegistryEntry, "value">
> = [
  {
    id: "smile",
    emoji: "😀",
...
export const DEFAULT_PROFILE_EMOJI_REGISTRY: PointRankEmojiRegistryEntry[] =
  DEFAULT_PROFILE_EMOJI_REGISTRY_BASE.map(buildEmojiValueAlias);
...
const normalizeLegacyValues = (
  rawLegacyValues: unknown,
  currentEmoji: string,
  rawValue: string,
) =>
  Array.from(
...
const getDefaultRegistryEntry = (index: number) =>
  DEFAULT_PROFILE_EMOJI_REGISTRY[index] || null;
...
const normalizeRegistryEntry = (
  ...
  const emoji = normalizeEmojiText(
    rawEntry?.emoji ?? rawEntry?.value ?? fallbackEntry?.emoji,
  );
  if (!emoji) return null;

  const rawValue = normalizeEmojiText(rawEntry?.value);
  const label = String(
    rawEntry?.label || fallbackEntry?.label || `이모지 ${index + 1}`,
  ).trim();
  const category =
    String(rawEntry?.category || fallbackEntry?.category || "기타").trim() ||
    "기타";
  ...
  const unlockTierCode =
    normalizeTierCode(rawEntry?.unlockTierCode) ||
    fallbackEntry?.unlockTierCode;

  return buildEmojiValueAlias({
    id: ensureUniqueId(
      String(rawEntry?.id || fallbackEntry?.id || label || emoji),
      usedIds,
      index,
    ),
    emoji,
    ...
    legacyValues: normalizeLegacyValues(
      rawEntry?.legacyValues,
      emoji,
      rawValue,
    ),
  ...
  const entries = rawRegistry
    .map((entry, index) =>
      normalizeRegistryEntry(
        entry as Partial<PointRankEmojiRegistryEntry>,
        index,
        usedIds,
      ),
    )
...
const getEnabledRegistryEntries = (registry: PointRankEmojiRegistryEntry[]) =>
...
export const getProfileEmojiRegistry = (
  ...
  const targetRegistry = options?.includeDisabled
    ? [...registry]
    : getEnabledRegistryEntries(registry);
...
export const getDefaultProfileEmojiValue = (
  registry: PointRankEmojiRegistryEntry[] = DEFAULT_PROFILE_EMOJI_REGISTRY,
) => {
  const defaultEntry =
    getProfileEmojiEntryById(DEFAULT_PROFILE_EMOJI_ID, registry) ||
...
const getClampedTierCode = (
  requestedTierCode: PointRankTierCode,
...
export const buildDefaultPointRankEmojiPolicy = (
  ...
  const tierUnlocks = tiers.reduce<PointRankEmojiPolicyTiers>(
    (accumulator, tier) => ({
      ...accumulator,
      [tier.code]: {
        allowedEmojiIds: [],
  ...
  enabledRegistry.forEach((entry) => {
    const requestedTierCode =
      normalizeTierCode(entry.unlockTierCode) || tiers[0]?.code || "tier_1";
