...
type RawScheduleCategory = Partial<ScheduleCategory> & {
  id?: string;
  hidden?: boolean;
};
...
export const COLOR_EMOJI_OPTIONS = CATEGORY_COLOR_PRESETS.map(
  (preset) => preset.emoji,
);
...
const DEFAULT_CATEGORY_MAP = new Map(
  DEFAULT_SCHEDULE_CATEGORIES.map((item) => [item.key, item]),
);
const scheduleCategoryListeners = new Set<
  (categories: ScheduleCategory[]) => void
>();
...
export const getColorForEmoji = (emoji: string, fallback = "#6b7280") =>
  CATEGORY_COLOR_PRESETS.find((preset) => preset.emoji === emoji)?.color ||
  fallback;
...
export const resolveScheduleCategories = (
  items?: RawScheduleCategory[],
): ScheduleCategory[] => {
  ...
  (items || []).forEach((item, index) => {
    ...
    normalized.set(key, {
      key,
      label:
        String(item?.label || fallback?.label || "일정").trim() ||
        fallback?.label ||
        "일정",
      color: normalizeColor(
        item?.color,
        getColorForEmoji(emoji, fallback?.color || "#6b7280"),
      ),
      emoji,
      order:
        typeof item?.order === "number"
          ? item.order
          : (fallback?.order ?? DEFAULT_SCHEDULE_CATEGORIES.length + index),
...
export const getScheduleCategoryMeta = (
  key: unknown,
  ...
  return (
    source.find((item) => item.key === categoryKey) ||
...
export const useScheduleCategories = () => {
  const [categories, setCategories] =
    useState<ScheduleCategory[]>(sharedCategories);
