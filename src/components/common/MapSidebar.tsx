...
const getMapSidebarIcon = (item: MapResource) => {
  ...
  if (item.type === "google" || source.includes("구글")) {
    return (
      <span
        className="text-sm font-extrabold leading-none text-blue-600"
        aria-hidden="true"
      >
        🅖
      </span>
          ...
          {items.map((item) => (
            ...
            >
              {reorderMode ? (
                ...
                <div className="w-6 text-center">{getMapSidebarIcon(item)}</div>
              )}
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-bold">{item.title}</div>
              </div>
              {renderItemAction && (
                <div className="ml-auto shrink-0">{renderItemAction(item)}</div>
              )}
            </button>
          ))}
        </nav>
