...
const dedupePreviewEmojiEntries = (entries: PointRankEmojiRegistryEntry[]) => {
  ...
  return entries.filter((entry) => {
    const normalizedEmojiId = String(entry.id || "")
      .trim()
      .toLowerCase();
...
const StudentRankPromotionPopup: React.FC<StudentRankPromotionPopupProps> = ({
  ...
  const previewCountLabel =
    dedupedPreviewEmojis.length > visiblePreviewEmojis.length
        ...
        <div className="relative overflow-y-auto overscroll-contain p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <h2
                id={titleId}
                className="mt-2 text-2xl font-black tracking-tight text-slate-900 sm:text-[2rem]"
              >
                등급이 상승했어요
              </h2>
              <p
          ...
          <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center">
            <div
              className={[
                ...
                effectLevel === "subtle"
                  ? "shadow-[0_10px_24px_rgba(148,163,184,0.16)]"
                  : "shadow-[0_18px_32px_rgba(245,158,11,0.18)]",
              ].join(" ")}
            >
              <div
                className={[
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-xs font-medium text-slate-500">
                현재 누적 위스 {rank.metricValue.toLocaleString("ko-KR")} ₩s
                기준으로 도달한 등급이에요.
              </p>
            </div>
          </div>
