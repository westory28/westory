...
interface StudentRankPromotionState {
  ...
  previewEmojiEntries: Awaited<
    ReturnType<typeof getStudentRankPromotionPreviewEmojiEntries>
  >;
}
...
const StudentRankPromotionController: React.FC = () => {
  ...
  const [promotionState, setPromotionState] =
    useState<StudentRankPromotionState | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const effectLevel =
          snapshot.policy.rankPolicy.celebrationPolicy?.effectLevel === "subtle"
            ? "subtle"
            : "standard";
        const celebrationEnabled =
          snapshot.policy.rankPolicy.celebrationPolicy?.enabled !== false;
        const currentTierCode = rank.tierCode;
        const storedTierCode = readStudentRankPromotionTierCode(
          config,
          currentUser.uid,
        );
