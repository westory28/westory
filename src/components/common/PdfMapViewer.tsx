...
const PdfMapViewer: React.FC<PdfMapViewerProps> = ({
  ...
  const shortcutSections = useMemo(() => {
    ...
    source.forEach((region) => {
      ...
      if (selectedTag) {
        if (tags.includes(selectedTag)) appendRegion(selectedTag, region);
        return;
      }
      if (tags.length === 0) {
    ...
    return sortTags(Array.from(groups.keys())).map((tag) => ({
      tag,
      items: [...(groups.get(tag) || [])].sort((a, b) => a.label.localeCompare(b.label, 'ko')),
    }));
  }, [filteredRegionHits, selectedTag, visibleRegionHits]);
  ...
  useEffect(() => {
    ...
    const containerWidth = Math.max(0, modalSurfaceRef.current.clientWidth - 32);
    const containerHeight = Math.max(0, modalSurfaceRef.current.clientHeight - 32);
    const nextFitZoom = clamp(Math.min(containerWidth / currentPageWidth, containerHeight / currentPageHeight), 0.35, 2.2);
  ...
  const handleSelectRegion = (region: RegionHit) => {
    ...
    setZoom((prev) => Math.max(prev, fitZoom * 1.6, 1.2));
  };
  ...
  const renderTagFilters = () => availableTags.length > 0 && (
    <div className="mb-4">
      <div className="mb-2 text-xs font-bold text-gray-500">태그 목차</div>
      ...
      {shortcutSections.length > 0 ? (
        <div className="space-y-4">
          {shortcutSections.map((section) => {
            ...
            return (
              <div key={section.tag} className="rounded-2xl border border-gray-200 bg-white/80 p-3">
  ...
  const renderPageSurface = (interactive: boolean) => {
    ...
    return (
      <div
        ...
        onClick={interactive ? () => { if (!loadingPdf && !selectedRegion) { setZoom(fitZoom); } setIsModalOpen(true); } : undefined}
      >
        ...
        {!loadingPdf && nativeViewerFallback && fileUrl && (
          <div className="space-y-3">
            <div className="rounded-xl bg-white px-4 py-4 text-sm text-amber-700">PDF 추출 보기에 실패해 브라우저 기본 뷰어로 대신 표시합니다.<div className="mt-2 text-xs text-gray-500">{loadError}</div></div>
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white"><iframe src={fileUrl} title={`${title} PDF`} className="h-[72vh] w-full" /></div>
          </div>
        )}
        {!loadingPdf && usingPreprocessedPages && currentPageImage && (
          <div className="relative inline-block">
            <img src={currentPageImage.imageUrl} alt={`${title} ${currentPage}페이지`} style={{ width: `${currentPageImage.width * surfaceScale}px`, maxWidth: 'none' }} />
        ...
        {!loadingPdf && !loadError && !usingPreprocessedPages && pdfSourceUrl && (
          <Document file={pdfSourceUrl} onLoadSuccess={handleLoadSuccess} onLoadError={(error) => setLoadError(error.message || 'pdf-render-failed')} loading="PDF를 불러오는 중입니다.">
            <div className="relative inline-block">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => { if (!loadingPdf && !selectedRegion) setZoom(fitZoom); setIsModalOpen(true); }} disabled={loadingPdf} className="rounded-lg border border-blue-200 px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-50 disabled:opacity-40">확대 보기</button>
        ...
        <p className="mt-2 text-xs leading-6 text-gray-500">태그 목차와 지역 목차는 저장된 좌표와 태그 기준으로 동작합니다.</p>
      </div>
      {isModalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/70 p-4" onClick={() => setIsModalOpen(false)}>
          <div className="flex h-[90vh] w-full max-w-[96vw] flex-col overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              <div><div className="text-lg font-extrabold text-gray-900">{title}</div><div className="text-xs text-gray-500">바깥 클릭 또는 Esc로 닫습니다.</div></div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => { setSelectedRegion(null); setZoom(fitZoom); modalSurfaceRef.current?.scrollTo({ left: 0, top: 0, behavior: 'smooth' }); }} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-bold text-gray-700 hover:bg-gray-50">전체 보기</button>
            ...
            <div className="grid min-h-0 flex-1 lg:grid-cols-[19rem_minmax(0,1fr)]">
              <aside className="min-h-0 border-r border-gray-200 bg-gray-50 p-4">{renderTagFilters()}<div className="max-h-[calc(90vh-12rem)] overflow-y-auto pr-1">{renderShortcutList('rounded-xl px-3 py-2 text-left text-xs font-bold transition')}</div></aside>
              <div className="flex min-h-0 flex-1 flex-col">
                <div className="flex items-center justify-center border-b border-gray-200 px-5 py-3"><div className="text-sm font-bold text-gray-600">{currentPage} / {numPages || '-'}</div></div>
                <div className="flex min-h-0 flex-1 items-center justify-center bg-slate-100 p-4">
                  <div className="flex h-full w-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                    <div ref={modalSurfaceRef} className="min-h-0 flex-1 cursor-grab overflow-auto bg-slate-100 p-4 active:cursor-grabbing" onWheel={handleWheelZoom} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={() => { pinchStateRef.current = null; }} onMouseDown={handleDragStart} onMouseMove={handleDragMove} onMouseUp={() => { dragStateRef.current = null; }} onMouseLeave={() => { dragStateRef.current = null; }}>
                      <div className="flex min-h-full min-w-max items-start justify-start">{renderPageSurface(false)}</div>
                    </div>
                  </div>
                </div>
              </div>
