...
const PRESET_ITEMS: Array<{
    ...
    { key: 'first', label: '1위 시상대', toneClassName: 'border-amber-300 bg-amber-100/95 text-amber-950' },
    { key: 'second', label: '2위 시상대', toneClassName: 'border-slate-300 bg-slate-100/95 text-slate-900' },
    { key: 'third', label: '3위 시상대', toneClassName: 'border-orange-300 bg-orange-100/95 text-orange-950' },
    { key: 'leaderboard', label: '오른쪽 랭킹 패널', toneClassName: 'border-sky-300 bg-sky-100/95 text-sky-950' },
];

const clamp = (value: number, minimum: number, maximum: number) => Math.min(maximum, Math.max(minimum, value));

const cloneEditorValue = (value?: WisHallOfFamePositionEditorValue): WisHallOfFamePositionEditorValue => ({
    positions: {
        desktop: {
            first: { ...(value?.positions.desktop.first || DEFAULT_EDITOR_VALUE.positions.desktop.first) },
            second: { ...(value?.positions.desktop.second || DEFAULT_EDITOR_VALUE.positions.desktop.second) },
            third: { ...(value?.positions.desktop.third || DEFAULT_EDITOR_VALUE.positions.desktop.third) },
        },
        mobile: {
            first: { ...(value?.positions.mobile.first || DEFAULT_EDITOR_VALUE.positions.mobile.first) },
            second: { ...(value?.positions.mobile.second || DEFAULT_EDITOR_VALUE.positions.mobile.second) },
            third: { ...(value?.positions.mobile.third || DEFAULT_EDITOR_VALUE.positions.mobile.third) },
        },
    },
    leaderboardPanel: {
        desktop: { ...(value?.leaderboardPanel.desktop || DEFAULT_EDITOR_VALUE.leaderboardPanel.desktop) },
        mobile: { ...(value?.leaderboardPanel.mobile || DEFAULT_EDITOR_VALUE.leaderboardPanel.mobile) },
    },
});

const WisHallOfFamePositionEditor: React.FC<WisHallOfFamePositionEditorProps> = ({
    value,
    imageUrl,
    deviceMode,
    disabled = false,
    ...
    const updatePosition = (key: EditableKey, updater: (current: HallOfFameLeaderboardPanelPosition) => HallOfFameLeaderboardPanelPosition) => {
        const nextValue = cloneEditorValue(editorValue);
        if (key === 'leaderboard') {
            nextValue.leaderboardPanel[deviceMode] = updater(nextValue.leaderboardPanel[deviceMode]);
        } else {
            nextValue.positions[deviceMode][key] = updater(nextValue.positions[deviceMode][key]);
        }
        onChange(nextValue);
    };
    ...
    useEffect(() => {
        ...
        const handlePointerMove = (event: PointerEvent) => {
            const deltaXPercent = ((event.clientX - dragState.startClientX) / dragState.stageWidth) * 100;
            const deltaYPercent = ((event.clientY - dragState.startClientY) / dragState.stageHeight) * 100;
    ...
    const handlePointerDown = (event: React.PointerEvent<HTMLButtonElement>, key: EditableKey) => {
    ...
    const handleWidthChange = (nextWidth: number) => {
        updatePosition(selectedKey, (current) => ({
            ...current,
            widthPercent: clamp(nextWidth, selectedKey === 'leaderboard' ? 40 : 12, selectedKey === 'leaderboard' ? 100 : 42),
        }));
    };
        ...
        <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <div className="inline-flex rounded-full bg-white p-1 shadow-sm">
                        <button
                            ...
                            className={`min-h-10 rounded-full px-4 text-sm font-black transition ${deviceMode === 'desktop' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            데스크톱
                        </button>
                        <button
                            ...
                            className={`min-h-10 rounded-full px-4 text-sm font-black transition ${deviceMode === 'mobile' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            모바일
                        </button>
                    </div>
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.25fr)_320px]">
                <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
                    ...
                    <div
                        ref={stageRef}
                        className={`relative aspect-[16/10] overflow-hidden bg-slate-100 ${disabled ? 'opacity-70' : ''}`}
                    >
                        <img
                        ...
                        {PRESET_ITEMS.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`absolute -translate-x-1/2 rounded-3xl border px-3 py-3 text-left shadow-[0_16px_32px_rgba(15,23,42,0.14)] backdrop-blur-md transition ${item.toneClassName} ${isSelected ? 'ring-4 ring-slate-900/15' : 'hover:ring-2 hover:ring-slate-900/10'} ${disabled ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'}`}
                                >
                                    {isLeaderboard ? (
                                        ...
                                        <div className="space-y-2 text-center">
                                            ...
                                            <div className="text-[30px] leading-none drop-shadow-[0_10px_18px_rgba(15,23,42,0.18)]">
                                                {item.key === 'first' ? '👑' : item.key === 'second' ? '🛡️' : '🏹'}
                                            </div>
                                            <div className="rounded-2xl bg-white/84 px-3 py-2 text-xs font-black text-slate-800">
                                                {item.key === 'first' ? '1위 최유진' : item.key === 'second' ? '2위 김현우' : '3위 박지안'}
                                            </div>
                                        </div>
                                    )}
                                </button>
                ...
                <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                        {PRESET_ITEMS.map((item) => (
                            <button
                                ...
                                type="button"
                                onClick={() => setSelectedKey(item.key)}
                                className={`rounded-full border px-3 py-2 text-sm font-bold transition ${selectedKey === item.key ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100'}`}
                            >
                                {item.label}
                            </button>
                        ))}
                    ...
                    <div className="mt-5 grid grid-cols-3 gap-3 text-sm">
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                            ...
                            <div className="mt-1 text-base font-black text-slate-900">{Math.round(selectedPosition.leftPercent)}%</div>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                            <div className="text-xs font-bold text-slate-500">세로</div>
                            <div className="mt-1 text-base font-black text-slate-900">{Math.round(selectedPosition.topPercent)}%</div>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                            <div className="text-xs font-bold text-slate-500">넓이</div>
                            <div className="mt-1 text-base font-black text-slate-900">{Math.round(selectedPosition.widthPercent)}%</div>
                        </div>
                    </div>

                    <div className="mt-5 rounded-2xl border border-dashed border-sky-200 bg-sky-50 px-4 py-4 text-sm text-sky-900">
                        오른쪽 랭킹 패널도 같은 방식으로 이동할 수 있습니다. 배경이 복잡한 이미지일수록 패널 폭과 위치를 함께 조절해 보세요.
                    </div>
                </div>
            </div>
        </div>
