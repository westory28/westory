...
const MapViewer: React.FC<MapViewerProps> = ({
  ...
  const effectiveGoogleQuery =
    item.type === "google"
      ? (googleSearchQuery ?? item.googleQuery ?? "").trim()
      : "";

  const googleEmbedUrl =
    item.type === "google" ? getGoogleMapsEmbedUrl(effectiveGoogleQuery) : "";
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex w-full flex-col items-stretch gap-3 lg:w-auto lg:min-w-[20rem] lg:items-end">
              ...
              {externalUrl && (
                ...
                >
                  <i className="fas fa-up-right-from-square"></i>새 창에서 열기
                </a>
              )}
            </div>
          </div>
      ...
      <div className={showShell ? "bg-gray-50 p-4 md:p-6" : "space-y-4"}>
        {!showShell && item.type === "google" && onGoogleSearchQueryChange && (
          <div className="flex justify-stretch sm:justify-end">
            {renderGoogleSearchInput("map-google-search-inline")}
          </div>
        )}
