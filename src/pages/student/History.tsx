...
const StudentExamAnswer: React.FC = () => {
  ...
  >({});
  const [subjectiveRevealed, setSubjectiveRevealed] = useState<
  ...
  const maxScore = useMemo(() => {
    ...
    const objectiveTotal = examConfig.objective.reduce(
    ...
    const subjectiveTotal = examConfig.subjective.reduce(
      (sum, parent) =>
        sum +
        (parent.subItems || []).reduce(
          (subSum, sub) => subSum + (Number(sub.score) || 0),
          0,
        ),
      0,
    );
  ...
  const hasAnyInput = useMemo(() => {
    ...
    const hasSubjective = examConfig.subjective.some(
      (parent) => (parent.subItems || []).length > 0,
    );
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-8">
        ...
        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
          <h2 className="font-bold text-lg text-gray-800 mb-4 border-b border-gray-100 pb-2">
            <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded mr-2">
              선택형
            </span>
            객관식 답안
          </h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="w-8 text-center text-blue-600 font-bold">
                        {index + 1}
                      </span>
                      <div className="flex gap-2">
                        {[1, 2, 3, 4, 5].map((choice) => {
                          ...
                          const isCorrectSelected =
                            isSelected && mark?.isCorrect;
                          const isWrongSelected =
                            isSelected && !mark?.isCorrect;
                          const isMissedAnswer =
                            !!mark && !mark.isCorrect && choice === answer;

                          return (
                            <button
                              key={choice}
                              onClick={() =>
                                handleObjectiveSelect(index, choice)
                              }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`w-6 text-center font-black ${mark ? (mark.isCorrect ? "text-green-600" : "text-red-500") : "text-gray-300"}`}
                      >
        ...
        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <h2 className="font-bold text-lg text-gray-800 mb-4 border-b border-gray-100 pb-2">
            <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-1 rounded mr-2">
              논술형
            </span>
            서술형 답안
          </h2>
          ...
          <div className="bg-yellow-50 text-yellow-800 text-sm p-3 rounded mb-4">
            ...
            정답을 확인한 후 <strong>정답 인정</strong> 또는{" "}
            <strong>오답</strong>을 선택하세요.
          </div>

          {examConfig?.subjective.some(
            (parent) => (parent.subItems || []).length > 0,
          ) ? (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span
                                className={`w-6 text-center font-black ${grade === undefined ? "text-gray-300" : grade ? "text-green-600" : "text-red-500"}`}
                              >
