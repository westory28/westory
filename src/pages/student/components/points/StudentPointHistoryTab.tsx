...
import { formatPointDateTime, formatWisAmount, formatWisDelta } from '../../../../lib/pointFormatters';
    ...
    <div className="space-y-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p className="mt-1 text-sm text-gray-500">획득과 사용 내역을 최신순으로 확인할 수 있습니다.</p>
            </div>
                ...
                <table className="w-full min-w-[860px] text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                        {transactions.length === 0 && (
                            <tr>
                                <td colSpan={5} className="px-4 py-10 text-center text-gray-400">조건에 맞는 위스 내역이 없습니다.</td>
                            </tr>
                        )}
                        {transactions.map((transaction) => {
                            ...
                            return (
                                <tr key={transaction.id} className="hover:bg-gray-50">
                                    ...
                                    <td className={`px-4 py-4 text-right font-black whitespace-nowrap ${getPointDeltaToneClass(transaction.delta)}`}>
                                        {formatWisDelta(transaction.delta)}
                                    </td>
                                    <td className="px-4 py-4 text-gray-700">{reason}</td>
                                    <td className="px-4 py-4 text-right font-bold text-gray-800 whitespace-nowrap">{formatWisAmount(transaction.balanceAfter)}</td>
                                </tr>
                            );
                        })}
                    </tbody>
