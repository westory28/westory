    ...
    <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {
            "\uD604\uC7AC \uD3EC\uC778\uD2B8\uB85C \uAD6C\uB9E4\uD560 \uC218 \uC788\uB294 \uC0C1\uD488\uC744 \uD655\uC778\uD558\uACE0 \uC694\uCCAD\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4."
          }
        </p>
      </div>
      <div className="text-sm font-bold text-gray-500">
        {"\uC0AC\uC6A9 \uAC00\uB2A5 \uD3EC\uC778\uD2B8 "}
        <span className="text-blue-700">{`${wallet.balance || 0}\uC810`}</span>
      </div>
    </div>
    ...
    {products.length > 0 && (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((product) => {
          ...
          const productStatusText = !hasStock
            ? "재고 없음"
            : !canAfford
              ? "포인트 부족"
              : "구매 가능";
          const productStatusClass =
            !hasStock || !canAfford ? "text-rose-500" : "text-emerald-600";
                ...
                <div className="flex flex-1 flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <div className="mt-1 text-base font-black text-blue-700 sm:text-lg">
                        {product.price}
                      </div>
                    </div>
                  </div>
                  <div className="mt-auto flex min-h-[20px] items-center justify-between gap-2 text-sm">
                    ...
                    <span
                      className={`text-right text-xs font-bold whitespace-nowrap sm:text-sm ${productStatusClass}`}
                    >
                      {productStatusText}
                    </span>
                  </div>
