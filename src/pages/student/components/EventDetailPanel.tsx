...
const EventDetailPanel: React.FC<EventDetailPanelProps> = ({
  ...
  const [detailModalEvent, setDetailModalEvent] =
        ...
        {!selectedDate ? (
          <div className="flex h-full flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-center text-sm">
              달력에서 날짜 또는 일정을 클릭하면 상세 내용을 확인할 수 있습니다.
            </p>
          </div>
        ) : events.length === 0 ? (
          ...
          events.map((event) => {
            ...
            const categoryMeta = getScheduleCategoryMeta(
              event.eventType,
              categories,
            );
            ...
            if (isHoliday) {
              return (
                <div
                  key={event.id}
                  className="group mb-3 rounded-r-lg border-l-4 border-red-400 bg-red-50 p-3"
                >
          ...
          >
            ...
            <div className="space-y-2 p-4">
              ...
              <p className="text-xs text-gray-500">
                ...
                detailModalEvent.start !== detailModalEvent.end
                  ? ` ~ ${detailModalEvent.end}`
                  : ""}
                <span className="ml-2 font-bold text-blue-600">
              ...
              <div className="rounded-lg border border-gray-100 bg-gray-50 p-3">
                <p className="whitespace-pre-wrap text-sm text-gray-700">
                  {detailModalEvent.description?.trim() ||
                    "상세 내용이 등록되지 않았습니다."}
                </p>
              </div>
            </div>
