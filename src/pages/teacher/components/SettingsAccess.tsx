  ...
  const saveUserAccess = async (user: UserRow, patch: Partial<UserRow>) => {
    ...
    const nextUser: UserRow = {
      ...
      staffPermissions: normalizeStaffPermissions(
        patch.staffPermissions ?? user.staffPermissions,
      ),
      teacherPortalEnabled:
        patch.teacherPortalEnabled ?? user.teacherPortalEnabled,
    };
        ...
        prev.map((item) =>
          item.id === user.id
            ? {
                ...item,
                role:
                  nextUser.staffPermissions.length > 0 ? "staff" : "student",
                teacherPortalEnabled: nextUser.teacherPortalEnabled,
                staffPermissions: nextUser.staffPermissions,
              }
            : item,
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="border-b border-gray-100 pb-4">
          ...
          <p className="mt-1 text-sm text-gray-500">
            로그인 허용 여부와 들어올 수 있는 메뉴를 직접 선택합니다. 등록,
            수정, 삭제는 관리자만 가능합니다.
          </p>
        </div>
                        ...
                        <td key={permission} className="px-4 py-3">
                          <label
                            className={`inline-flex items-center gap-2 ${
                              disabled || !user.teacherPortalEnabled
                          ...
                          >
                            <input
                              ...
                              disabled={
                                disabled ||
                                (!isAdmin && !user.teacherPortalEnabled)
                              }
                              onChange={() =>
