          ...
          results.map((result) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-[11px] font-bold ${statusMeta[result.status].className}`}
                >
                  {statusMeta[result.status].label}
                </span>
              </div>
              <div className="mt-2 text-xs text-gray-500">
                {result.score}/{result.total} · {result.percent}% · 기준{" "}
