    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                        ...
                        {products.map((product) => (
                            <tr key={product.id} className="transition hover:bg-gray-50">
                                <td className="p-4">
                                    ...
                                    <div className="mt-1 text-xs text-gray-500">{product.description || '설명이 아직 없습니다.'}</div>
                                </td>
        ...
        <form onSubmit={onSubmit} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <input value={productForm.imageUrl} onChange={(event) => onProductFormChange((prev) => ({ ...prev, imageUrl: event.target.value }))} placeholder="이미지 URL" className="rounded-lg border border-gray-300 px-4 py-2.5 text-sm" />
            ...
            <button type="submit" disabled={!canManage} className="mt-5 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-bold text-white disabled:bg-blue-300">
                상품 저장
            </button>
        </form>
    </div>
