    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-gray-100 p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            접힌 상태에서는 요약만 보고, 펼친 상태에서 상세 설정과 허용 이모지를
            함께 편집합니다.
          </p>
                    ...
                    <div className="mt-5 grid grid-cols-1 gap-5 xl:items-start xl:grid-cols-[minmax(0,1.35fr)_minmax(272px,0.88fr)]">
                      ...
                      <aside className="rounded-2xl border border-gray-200 bg-gray-50/80 p-4 sm:p-5">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <span className="inline-flex items-center rounded-full border border-gray-200 bg-white px-3 py-1 text-[11px] font-bold text-gray-500">
                            비활성 자동 잠금
                          </span>
                        </div>
                            ...
                            {draftRankPolicy.emojiRegistry.map((entry) => {
                              ...
                              return (
                                <button
                                  ...
                                  aria-label={`${tierPreview?.label || tier.code} 등급에 ${entry.emoji} ${
                                    checked
                                      ? "선택 해제"
                                      : disabled
                                        ? "선택 불가"
