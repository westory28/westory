...
const PointGrantTab: React.FC<PointGrantTabProps> = ({
  ...
  const requiresClassSelection =
    !isGlobalSearchActive && (gradeFilter === "all" || classFilter === "all");
          ...
          <div className="divide-y divide-gray-100">
            {requiresClassSelection ? (
              <div className="px-5 py-12 text-center text-sm text-gray-400">
                학년과 반을 먼저 선택하거나 이름으로 전역 검색해 주세요.
              </div>
            ) : loading ? (
              <div className="px-5 py-12 text-center text-sm text-gray-400">
                {isGlobalSearchActive
            ...
            {students.map((student) => {
              const rank = getPointRankDisplay({
                ...
                earnedPointsFromTransactions:
                  rankManualAdjustEarnedPointsByUid[student.uid] || 0,
              });
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-sm text-gray-500">
                  {formatPointStudentLabel(selectedStudent) || "소속 정보 없음"}
                </div>
              </div>
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-right">
                ...
                <div className="mt-1 whitespace-nowrap text-2xl font-black text-blue-700">
                  {formatWisAmount(currentBalance)}
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-gray-200 bg-gray-50/80 p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="inline-flex rounded-full border border-gray-200 bg-white p-1">
                  {(
                    ...
                    ] as Array<{ key: GrantMode; label: string; tone: string }>
                  ).map((item) => (
                    <button
                      key={item.key}
                      type="button"
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-gray-200 bg-white px-4 py-3">
                  ...
                  <div
                    className={`mt-1 whitespace-nowrap text-xl font-black ${projectedBalance < 0 ? "text-rose-500" : "text-gray-900"}`}
                  >
            ...
            <div className="mt-5 rounded-2xl border border-gray-200 bg-white p-4">
              ...
              {!manualAdjustEnabled && (
                <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-700">
                  운영 정책에서 교사 직접 지급 및 환수를 허용해야 반영할 수
                  있습니다.
                </div>
              )}
              ...
              {mode === "reclaim" && !allowNegativeBalance && (
                <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-600">
                  현재 정책에서는 잔액보다 큰 환수를 허용하지 않습니다. 학생
                  잔액을 넘는 금액은 서버에서 차단됩니다.
                </div>
              )}
              ...
              <form onSubmit={(event) => onSubmit(event, mode)}>
                <div className="grid grid-cols-1 gap-3">
                  <label className="block">
                    ...
                    <input
                      ...
                      disabled={
                        !canManage || !manualAdjustEnabled || isSubmitting
                      }
                    />
                  </label>
                  <label className="block">
                ...
                {!!feedback && (
                  <div
                    className={`mt-3 rounded-lg px-3 py-2 text-sm font-bold ${getPointFeedbackToneClass(feedback)}`}
                  >
                    {feedback}
                  </div>
                )}
