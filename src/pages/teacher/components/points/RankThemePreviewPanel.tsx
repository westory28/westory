...
const RankThemePreviewPanel: React.FC<RankThemePreviewPanelProps> = ({
  ...
  const compareThemes: Array<{
  ...
  }> = [
    {
      ...
      panelTone:
        "rounded-[1.8rem] border border-blue-200 bg-gradient-to-br from-blue-50 via-white to-slate-50 p-5 shadow-[0_20px_40px_-32px_rgba(37,99,235,0.48)]",
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              높은 등급이 위, 낮은 등급이 아래로 보이도록 정렬해 학생이 체감하는
              서열을 바로 비교합니다.
            </p>
          </div>
        ...
        {compareThemes.map((theme) => (
          <article key={theme.themeId} className={theme.panelTone}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span
                className={`inline-flex self-start whitespace-nowrap rounded-full border px-3 py-1 text-xs font-bold ${theme.chipTone}`}
              >
