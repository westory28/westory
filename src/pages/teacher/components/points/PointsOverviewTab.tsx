...
import { formatPointDateShortTime, formatPointDateTime, formatPointStudentLabel, formatWisAmount, formatWisDelta } from '../../../../lib/pointFormatters';
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_0.95fr]">
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h2 className="text-lg font-bold text-gray-800">학생별 위스 현황</h2>
                        <p className="mt-1 text-sm text-gray-500">학년, 반, 번호, 이름 검색을 조합해 학생 위스 현황과 최근 거래 내역을 빠르게 확인할 수 있습니다.</p>
                    </div>
                    ...
                    {wallets.map((wallet) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="text-right text-sm font-black text-blue-700 whitespace-nowrap">{formatWisAmount(wallet.balance || 0)}</div>
                                <div className="text-right text-sm font-bold text-emerald-600 whitespace-nowrap">{formatWisAmount(wallet.earnedTotal || 0)}</div>
                                <div className="text-right text-sm font-bold text-rose-500 whitespace-nowrap">{formatWisAmount(wallet.spentTotal || 0)}</div>
                            </button>
                        );
                    })}
                </div>
                ...
                {!selectedWallet ? (
                    ...
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-right">
                            ...
                            <div className="mt-1 whitespace-nowrap text-2xl font-black text-blue-700">{formatWisAmount(selectedWallet.balance || 0)}</div>
                        </div>
                    </div>

                    <div className="mt-5 grid grid-cols-2 gap-3">
                        <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                            <div className="text-xs font-bold text-gray-500">누적 획득 위스</div>
                            <div className="mt-1 whitespace-nowrap text-lg font-extrabold text-emerald-600">{formatWisAmount(selectedWallet.earnedTotal || 0)}</div>
                        </div>
                        <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                            <div className="text-xs font-bold text-gray-500">누적 사용 위스</div>
                            <div className="mt-1 whitespace-nowrap text-lg font-extrabold text-rose-500">{formatWisAmount(selectedWallet.spentTotal || 0)}</div>
                        </div>
                    </div>
                    ...
                    <div className="mt-5">
                        <div className="mb-3 flex items-center justify-between gap-3">
                            ...
                            {canManage && (
                                <div className="text-[11px] font-bold text-gray-400">최신 교사 직접 지급/환수 항목만 수정·취소 가능</div>
                            )}
                        </div>
                        <div className="max-h-[320px] overflow-y-auto rounded-xl border border-gray-200">
                            ...
                            {transactions.map((transaction, index) => {
                                ...
                                const isEditable = canManage && ['manual_adjust', 'manual_reclaim'].includes(transaction.type) && index === 0;
                                ...
                                return (
                                    <div key={transaction.id} className="border-b border-gray-100 p-4 last:border-0">
                                        ...
                                        >
                                            ...
                                            <div className="mt-1 text-xs text-gray-500">{`잔액 ${formatWisAmount(transaction.balanceAfter)} · ${formatPointDateTime(transaction.createdAt)}`}</div>
                                        </button>
                                        {isSelected && isEditable && (
                                            <div className="mt-3 rounded-xl border border-blue-100 bg-blue-50 p-3">
                                                <div className="flex flex-col gap-3 md:flex-row md:items-end">
                                                    <label className="flex-1">
                                                        <div className="mb-1 text-xs font-bold text-blue-700">수정 위스</div>
                                                        <input
                                                            type="number"
