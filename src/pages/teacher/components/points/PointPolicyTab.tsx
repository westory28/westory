...
const scrollPolicySectionIntoView = (sectionId: string) => {
        ...
        <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
                ...
                <nav className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                    {policySections.map((item) => (
                        <button
                            key={item.id}
                            type="button"
                            onClick={() => scrollPolicySectionIntoView(item.id)}
                            className="block w-full border-b border-slate-200 px-4 py-4 text-left last:border-b-0 hover:bg-blue-50 focus:bg-blue-50 focus:outline-none"
                        >
                            <span className="block text-sm font-extrabold text-slate-900">
                                {item.label}
                            </span>
            ...
            <div className="space-y-5">
                ...
                <SectionPanel
                    id="policy-control"
                    label="C. 운영 제어"
                    description="교사 직접 조정 가능 여부와 잔액 차감 정책을 보수적으로 제어합니다."
                    badge="서버 차단 정책"
                >
                    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        <PolicyToggle
