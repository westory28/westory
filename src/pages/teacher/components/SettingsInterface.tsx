...
const SettingsInterface: React.FC = () => {
  ...
  const [parentDraft, setParentDraft] = useState<
    Record<PortalType, { name: string; url: string }>
  >({
    student: { name: "", url: "" },
    teacher: { name: "", url: "" },
  });
    ...
    const loadSettings = async () => {
      try {
        ...
        if (interfaceSnap.exists()) {
          ...
          const resolvedHallOfFameConfig =
            resolveHallOfFameInterfaceConfig(data);
          setConfig({
            mainEmoji: (
              data.mainEmoji || DEFAULT_INTERFACE_CONFIG.mainEmoji
            ).trim(),
            mainSubtitle: (
              data.mainSubtitle || DEFAULT_INTERFACE_CONFIG.mainSubtitle
            ).trim(),
  ...
  const updatePortalMenus = (
    portal: PortalType,
    updater: (menus: MenuConfig[PortalType]) => MenuConfig[PortalType],
  ) => {
    setMenuConfig((prev) => ({
      ...prev,
  ...
  const moveParent = (portal: PortalType, index: number, direction: -1 | 1) => {
    updatePortalMenus(portal, (menus) =>
      moveInArray(menus, index, index + direction),
    );
  };
  ...
  const getChildDraftKey = (portal: PortalType, parentIndex: number) =>
    `${portal}-${parentIndex}`;
  ...
  const saveInterfaceConfig = async () => {
    ...
    try {
      await setDoc(
        ...
        {
          mainEmoji:
            config.mainEmoji.trim() || DEFAULT_INTERFACE_CONFIG.mainEmoji,
          mainSubtitle:
            config.mainSubtitle.trim() || DEFAULT_INTERFACE_CONFIG.mainSubtitle,
        ...
        {activeTab === "landing" && (
          <div className="max-w-3xl space-y-8">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 lg:p-8 shadow-sm">
              ...
              <div
                className={`space-y-6 transition-opacity duration-200 ${config.ddayEnabled ? "" : "opacity-50 pointer-events-none"}`}
              >
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">
                    제목
            ...
            <div className="border border-gray-200 rounded-xl p-4 bg-gray-50">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_auto] gap-2">
                <input
                  ...
                  onChange={(e) =>
                    updateParentDraft(activePortal, "name", e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateParentDraft(activePortal, "url", e.target.value)
                  }
                  ...
                  <div className="p-4 bg-gray-50 border-b border-gray-200">
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          moveParent(activePortal, parentIndex, -1)
                        }
                      ...
                      <button
                        ...
                        disabled={
                          parentIndex === menuConfig[activePortal].length - 1
                        }
                  ...
                  <div className="p-4 space-y-3 bg-white">
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_auto] gap-2">
                      <input
                        ...
                        value={
                          childDrafts[
                            getChildDraftKey(activePortal, parentIndex)
                          ]?.name || ""
                        }
                        onChange={(e) =>
                          updateChildDraft(
