    ...
    try {
      ...
      try {
        const treeSnap = await getDoc(
          doc(db, curriculumCollectionPath, "tree"),
        );
        const tree = treeSnap.exists()
          ? (treeSnap.data() as any).tree || []
          : [];
      ...
      const questionIds = Array.from(
        new Set(wrongLogs.map((item) => item.qid).filter(Boolean)),
      );
      ...
      wrongLogs.forEach((log) => {
        ...
        items.push({
          ...
          explanation: String(
            questionDoc.explanation || "해설이 등록되지 않았습니다.",
          ),
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-xs text-gray-500">
              학생 이름을 클릭하면 오답 기록과 문항 해설을 바로 확인할 수
              있습니다.
            </p>
                    ...
                    {items.map((item) => (
                      <div key={item.key}>
                        ...
                        >
                          ...
                          <i
                            className={`fas fa-chevron-down text-gray-400 transition ${expandedKey === item.key ? "rotate-180" : ""}`}
                          ></i>
                        </button>
