...
import {
  ...
  isTeacherUser as hasTeacherIdentity,
...
const Login: React.FC = () => {
  ...
  const stagingTestLoginEnabled =
    STAGING_TEST_LOGIN_ENABLED && searchParams.get("stagingTestLogin") === "1";
  ...
  const finishLoginForRole = async (user: User, mode: LoginMode) => {
    ...
    const nextRole: UserData["role"] =
      mode === "teacher"
        ? hasTeacherIdentity(existing, user.email || "")
          ? "teacher"
          : "staff"
        : "student";
    ...
    const nextStaffPermissions = nextRole === "student" ? [] : staffPermissions;
