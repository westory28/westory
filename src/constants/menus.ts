...
const getLegacyCanonicalUrl = (pathOnly: string) => {
  if (
    pathOnly === "/student/quiz/history2" ||
    pathOnly.startsWith("/student/quiz/history2/")
  ) {
    return "/student/quiz";
  }
  if (
    pathOnly === "/teacher/quiz/history2" ||
    pathOnly.startsWith("/teacher/quiz/history2/")
  ) {
    return "/teacher/quiz";
  }
  return "";
...
const normalizeMenuUrl = (value: unknown) => {
  ...
  const normalizedPath =
    pathPart.replace(/\/{2,}/g, "/").replace(/\/+$/, "") || "/";
  const canonicalPath = getLegacyCanonicalUrl(normalizedPath) || normalizedPath;
  return queryPart && canonicalPath === normalizedPath
    ? `${canonicalPath}?${queryPart}`
    : canonicalPath;
};
...
export const sanitizeMenuConfig = (raw: unknown): MenuConfig => {
  ...
  const parsePortal = (portal: PortalType) => {
    ...
    const missingParents = fallback[portal].filter(
      (fallbackItem) =>
        !withFallbackChildren.some((item) => item.url === fallbackItem.url),
    );
